...
async def on_message(message):
    ...
    Handles AI responses when the bot is mentioned.
    """
    ...
    if model is None:
        # Only reply if mentioned, to avoid spamming channels.
        if client.user.mentioned_in(message):
            print(f"Warning: Received message from {message.author} but Gemini model is not initialized.")
    ...
    if client.user.mentioned_in(message):
        ...
        try:
            ...
            async with message.channel.typing():
                print(f"Sending prompt to Gemini: '{user_message}' from {message.author}")
            ...
            if "Quota exceeded" in error_message_str:
                ...
                # If the model becomes unavailable/unsupported, inform the user and log.
                # The bot will need to be restarted to re-initialize the model via on_ready.
