...
import json
import datetime
import urllib.parse # <-- Added for the new !price command
...
async def price(ctx: commands.Context, *, game_name: str = None):
    ...
    try:
        async with ctx.typing():
            # First, check if there's a deal for the game
            response = requests.get(DEAL_API_URL)
            response.raise_for_status()
            deals_data = response.json()

            # --- CASE 1: A DEAL IS FOUND ---
            if deals_data:
                deal = deals_data[0]
                title = deal.get('title', 'Unknown Game')
                normal_price = deal.get('normalPrice', 'N/A')
            ...
            # --- CASE 2: NO DEAL FOUND, CHECK REGULAR PRICE ---
            else:
