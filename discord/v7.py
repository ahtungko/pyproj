...
# Updates
# Fix horoscope date
# Changed !cat command to !c
# Corrected !c command to fetch a direct image file instead of a webpage.
# Final fix for !c to use a permanent image link, preventing the image from changing on click.
# Switched !c command to use thecatapi.com for better reliability.
...
async def fetch_and_send_horoscope(destination, sign, user: discord.User = None):
    url = f"https://horoscope-app-api.vercel.app/api/v1/get-horoscope/daily?sign={sign}&day=TOMORROW"
    try:
...
async def on_ready():
    ...
    ai_personality = (
        "You are a confident, charming, and very flirty AI. Your goal is to be both helpful and alluring. "
        "You enjoy witty banter, give clever compliments to the user, and often use winking emojis like 😉 or 😏."
    )
    # -------------------------------------------

    if GEMINI_API_KEY:
        try:
            # Add the 'system_instruction' parameter to the model initialization
...
async def c(ctx: commands.Context):
    ...
    try:
        async with ctx.typing():
            ...
            # The API returns a list with one image object.
            if not data:
                await ctx.send("The cat API returned no cats. This is a catastrophe! 😿")
                return

            # Get the direct URL to the image file.
...
async def cf(ctx: commands.Context):
    ...
    try:
        async with ctx.typing():
            ...
            # The API returns {"data": ["fact"]}, so we extract the fact.
